...
import { loadAll, logActivity, pushChanges, subscribeAll } from "./db";
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  // Theme is a per-device preference.
  useEffect(() => {
  ...
  useEffect(() => {
    if (hydrated) localStorage.setItem(THEME_KEY, state.theme);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (userId) stop = subscribeAll(() => void refresh(userId));
    })();
...
export const CAN: Record<string, Role[]> = {
  dashboard: ["ADMIN", "STORE_MANAGER", "EMPLOYEE", "ACCOUNTANT", "VIEWER"],
  purchases: ["ADMIN", "STORE_MANAGER", "EMPLOYEE", "ACCOUNTANT", "VIEWER"],
  inventory: ["ADMIN", "STORE_MANAGER", "EMPLOYEE", "VIEWER"],
  expenses: ["ADMIN", "STORE_MANAGER", "EMPLOYEE", "ACCOUNTANT", "VIEWER"],
  tasks: ["ADMIN", "STORE_MANAGER", "EMPLOYEE", "MECHANIC", "VIEWER"],
  invoices: ["ADMIN", "STORE_MANAGER", "ACCOUNTANT"],
  notifications: ["ADMIN", "STORE_MANAGER", "EMPLOYEE", "MECHANIC", "ACCOUNTANT", "VIEWER"],
  messages: ["ADMIN", "STORE_MANAGER", "EMPLOYEE", "MECHANIC", "ACCOUNTANT"],
  partnersChat: ["ADMIN", "STORE_MANAGER"],
  earnings: ["ADMIN", "STORE_MANAGER", "MECHANIC", "ACCOUNTANT"],
  reports: ["ADMIN", "STORE_MANAGER", "ACCOUNTANT", "VIEWER"],
  users: ["ADMIN"],
  settings: ["ADMIN"],
  exports: ["ADMIN", "STORE_MANAGER", "ACCOUNTANT"],
  ...
  write: ["ADMIN", "STORE_MANAGER", "EMPLOYEE", "MECHANIC", "ACCOUNTANT"],
};
